...
function checkoutAuthorizationGuidance(checkout = {}) {
  ...
  return {
    ...
    next_step: checkout.next_required_action === "auth_qr" || checkout.identity_status === "waiting_human_auth"
      ? "展示首次购买授权入口，然后继续 resume 同一个 checkout；这是授权，不是付款。"
      : checkout.payment_intent_id || checkout.agent_next_actions?.includes("create_payment_intent") || checkout.next_required_action === "create_payment_intent"
        ? "checkout 已允许进入付款；继续创建或展示 payment_intent。"
        : hasSession
          ? "已有有效 buyer session；继续同一个 checkout。"
          : "继续 resume/status 同一个 checkout，直到服务端给出下一步动作。",
    do_not: [
      "不要把 auth_qr 说成付款二维码或付款证明。",
      "不要把本地 buyer session 缺失当作公开 checkout 的付款 blocker。",
