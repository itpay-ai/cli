...
async function renderItPayPaymentAction(intent, flags = {}) {
  ...
  if (action && (intent?.qr_image_url || intent?.qr?.image_url)) {
    ...
    action.description = action.description || "Scan the ItPay payment entry QR; ItPay will safely hand off to Alipay or WeChat Pay.";
  }
...
async function renderHumanAction(action, flags = {}) {
  ...
  if (mode === "file" || flags.qr_file || process.env.ITP_QR_FILE) {
    ...
    if (qrImageURL) {
      ...
      process.stderr.write(`${providerLabel} branded QR image: ${file}\n`);
      renderResult.rendered = true;
  ...
  if (qrImageURL) {
    process.stderr.write(`${providerLabel} branded QR image URL: ${qrImageURL}\n`);
...
function humanActionTitle(action = {}) {
  ...
  return `Scan ${humanActionProviderLabel(action)} QR`;
}
