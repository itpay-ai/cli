...
function humanActionMarkdown(action = {}, { localQRPath = "", qrPNGURL = "", preferredQRURL = "", entryURL = "", mobileWalletURL = "" } = {}) {
  ...
  const sandboxAlipay = action.kind === "payment_qr" && [qrPNGURL, preferredQRURL, entryURL, mobileWalletURL]
    .some((value) => String(value || "").includes("alipay-sandbox") || String(value || "").startsWith("alipays://"));
  const lines = [action.kind === "auth_qr" ? "请打开 ItPay 授权入口：" : (sandboxAlipay ? "请使用支付宝沙箱 App 扫码付款：" : "请扫码付款：")];
