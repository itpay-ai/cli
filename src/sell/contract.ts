...
export const SELL_GUIDE = {
    ...
    rules: [
        ...
        "Six human gates G1..G6 are enforced (identity, sources, workflow/fixtures/version, evidence, pricing, terms/submission). A gate_required response means: stop, show the review payload, wait for the user's decision, then run the printed `sell gates approve` command with their words. Never self-approve or use ITPAY_SELL_SKIP_GATES outside offline tests.",
...
export const SELL_OPERATIONS: SellOperation[] = [
    ...
    { command: "sources library", method: "POST", path: `${org}/api-intakes/from-library`, scope: write, fields: { library_api_id: "string", credential_source: "string" }, optional: ["credential_source"], description: "Bind a platform library source; credential_source is 'seller' (BYOK, default) or 'itpay' (platform-managed key)" },
    ...
    { command: "kyb status", method: "GET", path: `${org}/kyb`, scope: read, fields: {}, description: "Read KYB cases and merchant admission state" },
    { command: "kyb submit", method: "POST", path: `${org}/kyb`, scope: write, fields: { subject_type: "string", legal_name: "string", country: "string", contact_email: "string", evidence_url: "string", business_license_asset_id: "string", registration_number: "string", representative: "string" }, optional: ["business_license_asset_id", "registration_number", "representative", "evidence_url"], description: "Submit a KYB application for merchant verification; company applications require a business license asset uploaded via the dashboard", confirmation: true },
    ...
    { command: "fixtures suggest", method: "POST", path: `${draft}/fixtures/suggest`, scope: test, fields: { expected_semantic_revision: "number", instruction: "string", api_intake_ids: "array", operation_refs: "array", complete_existing: "string" }, optional: ["operation_refs", "complete_existing", "instruction"], description: "AI-generate valid test inputs matching API parameter contracts" },
