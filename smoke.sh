...
printf '%s' "$BUY_CODEX_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "waiting_user_payment" || json.must_reply_to_human_before_next_command !== true || !json.human_visible_markdown.includes("![ItPay payment QR](") || !json.next.command.includes("buyer payment wait pi_mock_pubg --timeout 60")) process.exit(1);})'
...
printf '%s' "$PAYMENT_WAIT_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (!json.payment_guidance || json.payment_guidance.payment_intent_id !== "pi_mock_pubg" || !json.payment_guidance.do_not.includes("不要因为 timeout、断网或没回应创建新 checkout。")) process.exit(1);})'
