...
printf '%s' "$AUTH_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; const msg=selected && selected.openclaw_message; const out=json.human_output; if (!selected || selected.platform !== "telegram" || selected.format !== "text_inline_buttons") process.exit(1); if (!out || out.method !== "openclaw_message_send" || out.channel !== "telegram" || !out.media || !out.presentation) process.exit(1); if (!selected.media.length || !selected.media[0].local_path) process.exit(1); if (!selected.links.length || !selected.links[0].url.includes("/checkouts/chk_mock_auth")) process.exit(1); if (selected.buttons.map((button)=>button.intent || button.url).length < 2) process.exit(1); if (!selected.text.includes("授权")) process.exit(1); const blocks=out.presentation.blocks || []; if (!msg || msg.media !== selected.media[0].local_path || !blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text && button.url && button.url.includes("/checkouts/chk_mock_auth")))) process.exit(1); if (!blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text === "我已完成，查询状态" && button.callback_data === "itp:check_checkout_status:chk_mock_auth"))) process.exit(1); if (!out.callbacks || out.callbacks.check_checkout_status !== "itp buyer checkout resume chk_mock_auth --json") process.exit(1); if (JSON.stringify(out).includes("MEDIA:") || JSON.stringify(out).includes("\"label\"")) process.exit(1);})'
...
printf '%s' "$BUY_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; const msg=selected && selected.openclaw_message; const out=json.human_output; if (!selected || selected.platform !== "telegram" || selected.format !== "photo_text_inline_buttons") process.exit(1); if (!out || out.method !== "openclaw_message_send" || out.channel !== "telegram") process.exit(1); if (!selected.media.length || selected.media[0].fallback_url !== "http://127.0.0.1/mock-qr.png") process.exit(1); if (!selected.links.some((link)=>link.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (selected.buttons.map((button)=>button.intent).join(",") !== "refresh_payment_qr,check_payment_status") process.exit(1); if (!msg || msg.media !== "http://127.0.0.1/mock-qr.png" || out.media !== msg.media) process.exit(1); const blocks=out.presentation && out.presentation.blocks || []; if (!blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text && button.url === "http://127.0.0.1/mock-pay"))) process.exit(1); if (!blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text === "我已付款，查询状态" && button.callback_data === "itp:check_payment_status:pi_mock_pubg"))) process.exit(1); if (!out.callbacks || out.callbacks.check_payment_status !== "itp buyer payment wait pi_mock_pubg --timeout 1 --json") process.exit(1); if (JSON.stringify(out).includes("MEDIA:") || JSON.stringify(out).includes("\"label\"") || JSON.stringify(out).includes("\"action\"")) process.exit(1);})'
