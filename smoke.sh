...
if HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buy var_pubg_couple_skin_cny20 --sandbox --email cli-buyer@itpay.ai --json >/dev/null 2>&1; then
  echo "buyer buy accepted deprecated --sandbox" >&2
  exit 1
fi
BAD_EMAIL="buyer@""example.com"
if HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buyer checkout create --cart cart_mock_pubg --email "$BAD_EMAIL" --json >/dev/null 2>&1; then
  echo "buyer checkout accepted placeholder email" >&2
  exit 1
fi
BUY_BLOCKED_OUTPUT=$(HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buy var_pubg_couple_skin_cny20 --email cli-buyer@itpay.ai --phone +8613800000000 --no-wait --json)
...
printf '%s' "$AUTH_QR_OUTPUT" | node -e 'const fs=require("fs");let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const action=json.checkout && json.checkout.human_action; const selected=json.render_plan && json.render_plan.selected; if (json.status !== "checkout_created" || !action || action.kind !== "auth_qr") process.exit(1); if (!action.local_qr_path || !fs.existsSync(action.local_qr_path)) process.exit(1); if (action.local_qr_mime !== "image/png") process.exit(1); if (action.agent_display_hint.primary !== "local_qr_path") process.exit(1); if (!json.human_output_required || !json.must_send_human_output_before_next_command || !json.human_output) process.exit(1); if (!json.render_plan || json.render_plan.kind !== "auth_qr" || json.render_plan.proof_rule !== "This human action is not payment proof." || !selected) process.exit(1); if (!json.render_plan.platforms.telegram.buttons.length || !json.render_plan.platforms.telegram.openclaw_message) process.exit(1); if (action.qr_png_url || action.qr_image_url || action.preferred_qr_url) process.exit(1);})'
AUTH_TELEGRAM_OUTPUT=$(HOME="$TMP_HOME" ITP_HOST=telegram ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buyer checkout create --cart cart_mock_auth --json)
printf '%s' "$AUTH_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const instruction=json.agent_instruction; const msg=instruction&&instruction.openclaw_message; const args=msg&&msg.command_args; const blocks=args&&args.presentation&&args.presentation.blocks||[]; const buttons=blocks.find((b)=>b.type==="buttons")&&blocks.find((b)=>b.type==="buttons").buttons||[]; if (json.status !== "checkout_created" || json.render_plan || json.human_output || json.human_visible_markdown || json.payment_handoff) process.exit(1); if (!json.must_send_to_human_before_next_command || !instruction || instruction.platform !== "telegram" || instruction.type !== "send_telegram_auth_handoff") process.exit(1); if (!args || args.channel !== "telegram" || !args.media || !args.message.includes("购物车和结算已创建") || !args.message.includes("首次授权/付款")) process.exit(1); if (!msg.telegram_rule.includes("openclaw message send") || !msg.telegram_rule.includes("Inline Keyboard")) process.exit(1); if (!buttons.some((button)=>button.text === "🔓 打开授权" && button.url && button.url.includes("/checkouts/chk_mock_auth"))) process.exit(1); if (!buttons.some((button)=>button.text === "✅ 我已完成，查询状态" && button.callback_data === "itp:check_checkout_status:chk_mock_auth")) process.exit(1); if (json.after_human_response.check_status_command !== "itp buyer checkout resume chk_mock_auth --json") process.exit(1); if (JSON.stringify(json).includes("MEDIA:") || JSON.stringify(json).includes("\"label\"")) process.exit(1);})'
...
JS
BUY_OUTPUT=$(HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buy var_pubg_couple_skin_cny20 --email cli-buyer@itpay.ai --phone +8613800000000 --no-wait --json)
printf '%s' "$BUY_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "payment_handoff_required" || json.cart.cart_id !== "cart_mock_pubg" || json.checkout.checkout_id !== "chk_mock_pubg" || json.payment_intent.payment_intent_id !== "pi_mock_pubg") process.exit(1); if (JSON.stringify(json.selection).includes("ai.itpay")) process.exit(1); if (json.payment_intent.human_action.preferred_qr_url !== "http://127.0.0.1/mock-qr.png") process.exit(1); if (json.payment_intent.human_action.agent_display_hint.primary !== "qr_png_url") process.exit(1); if (json.payment_intent.human_action.mobile_wallet_url !== "http://127.0.0.1/mock-mobile-wallet") process.exit(1); if (json.must_reply_to_human_before_next_command !== true || json.must_send_human_output_before_next_command !== true || !json.human_output_required || !json.human_visible_markdown.includes("![ItPay payment QR](")) process.exit(1); if (json.next.safe_for_agent !== false || json.next.type !== "reply_to_human") process.exit(1); if (!json.after_human_response || !json.after_human_response.check_payment_command.includes("buyer payment wait pi_mock_pubg --timeout 1 --json")) process.exit(1); if (!json.payment_handoff || !json.payment_handoff.markdown.includes("mock-qr.png") || !json.payment_handoff.markdown.includes("mock-pay")) process.exit(1); if (!json.render_plan || json.render_plan.kind !== "payment_qr" || json.render_plan.proof_rule !== "Only payment_intent.verified proves payment." || !json.render_plan.selected) process.exit(1); const tg=json.render_plan.platforms.telegram; if (!tg.media.length || tg.buttons.map(b=>b.intent).join(",") !== "refresh_payment_qr,check_payment_status") process.exit(1); if (json.agent_next_actions[0] !== "show_human_visible_markdown") process.exit(1); if (!json.docs.some((doc)=>doc.topic==="payment-wait")) process.exit(1); if (JSON.stringify(json).includes("issue_payment_proof")) process.exit(1);})'
BUY_CODEX_OUTPUT=$(HOME="$TMP_HOME" ITP_HOST=codex ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buy var_pubg_couple_skin_cny20 --email cli-buyer@itpay.ai --phone +8613800000000 --json)
printf '%s' "$BUY_CODEX_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; if (json.status !== "payment_handoff_required" || json.must_reply_to_human_before_next_command !== true || !json.human_visible_markdown.includes("![ItPay payment QR](") || json.next.safe_for_agent !== false) process.exit(1); if (!json.render_plan || json.render_plan.host !== "codex" || !json.render_plan.platforms.codex_app.markdown.includes("mock-qr.png")) process.exit(1); if (!selected || selected.platform !== "codex_app" || !selected.markdown.includes("mock-qr.png")) process.exit(1); if (!json.human_output || json.human_output.method !== "markdown_reply" || !json.human_output.markdown.includes("mock-qr.png")) process.exit(1);})'
BUY_TELEGRAM_OUTPUT=$(HOME="$TMP_HOME" ITP_HOST=telegram ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buy var_pubg_couple_skin_cny20 --email cli-buyer@itpay.ai --phone +8613800000000 --no-wait --json)
printf '%s' "$BUY_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const instruction=json.agent_instruction; const msg=instruction&&instruction.openclaw_message; const args=msg&&msg.command_args; const blocks=args&&args.presentation&&args.presentation.blocks||[]; const buttons=blocks.find((b)=>b.type==="buttons")&&blocks.find((b)=>b.type==="buttons").buttons||[]; if (json.status !== "payment_handoff_required" || json.render_plan || json.human_output || json.human_visible_markdown || json.payment_handoff || json.payment_guidance) process.exit(1); if (!json.order_summary || json.order_summary.payment_intent_id !== "pi_mock_pubg" || json.order_summary.amount_display !== "¥20.00") process.exit(1); if (!instruction || instruction.platform !== "telegram" || instruction.type !== "send_telegram_payment_handoff" || !instruction.proof_rule.includes("verified")) process.exit(1); if (!args || args.channel !== "telegram" || args.media !== "http://127.0.0.1/mock-qr.png" || !args.message.includes("购物车和结算已创建") || !args.message.includes("完成付款")) process.exit(1); if (!buttons.some((button)=>button.text === "💳 打开付款页面" && button.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (!buttons.some((button)=>button.text === "📱 手机钱包打开" && button.url === "http://127.0.0.1/mock-mobile-wallet")) process.exit(1); if (!buttons.some((button)=>button.text === "🔄 支付遇到问题 / 刷新" && button.callback_data === "itp:refresh_payment_qr:pi_mock_pubg")) process.exit(1); if (!buttons.some((button)=>button.text === "✅ 我已付款，查询状态" && button.callback_data === "itp:check_payment_status:pi_mock_pubg")) process.exit(1); if (json.after_human_response.check_status_command !== "itp buyer payment wait pi_mock_pubg --timeout 1 --json") process.exit(1); if (JSON.stringify(json).includes("MEDIA:") || JSON.stringify(json).includes("\"label\"") || JSON.stringify(json).includes("\"action\"")) process.exit(1);})'
STALE_TELEGRAM_OUTPUT=$(HOME="$TMP_HOME" ITP_HOST=telegram ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buyer payment wait pi_mock_stale --json)
printf '%s' "$STALE_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const args=json.agent_instruction&&json.agent_instruction.openclaw_message&&json.agent_instruction.openclaw_message.command_args; const blocks=args&&args.presentation&&args.presentation.blocks||[]; const buttons=blocks.find((b)=>b.type==="buttons")&&blocks.find((b)=>b.type==="buttons").buttons||[]; if (json.status !== "payment_handoff_required" || json.render_plan || json.human_output || json.payment_handoff) process.exit(1); if (!args || args.media !== "http://127.0.0.1/mock-stale-qr.png" || !buttons.some((button)=>button.url === "http://127.0.0.1/mock-stale-pay")) process.exit(1); if (!buttons.some((button)=>button.callback_data === "itp:check_payment_status:pi_mock_stale")) process.exit(1);})'
BUY_UNKNOWN_OUTPUT=$(HOME="$TMP_HOME" ITP_HOST=unknown_host ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buy var_pubg_couple_skin_cny20 --email cli-buyer@itpay.ai --phone +8613800000000 --no-wait --json)
printf '%s' "$BUY_UNKNOWN_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; const out=json.human_output; if (!selected || selected.platform !== "current_channel_required" || !selected.telegram || !selected.telegram.openclaw_message || !selected.fallback) process.exit(1); if (!out || out.method !== "current_channel_required" || !out.channels.openclaw_telegram || out.channels.openclaw_telegram.method !== "openclaw_message_send" || !out.channels.codex_app.markdown) process.exit(1); if (selected.fallback.format !== "image_or_link_then_human_reply") process.exit(1); if (!selected.fallback.links.some((link)=>link.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (!selected.fallback.text.includes("扫码")) process.exit(1);})'
SETUP_FLAG_VALIDATION=$(HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" setup --plan credit-300 --credits 20 --method alipay --json 2>&1 >/dev/null || true)
printf '%s' "$SETUP_FLAG_VALIDATION" | grep -q "use either --plan or --credits"
PAYMENT_WAIT_OUTPUT=$(HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buyer payment wait pi_mock_pubg --timeout 1 --json)
...
PAYMENT_WAIT_HANDOFF_OUTPUT=$(HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buyer payment wait pi_mock_pubg --json)
printf '%s' "$PAYMENT_WAIT_HANDOFF_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "payment_handoff_required" || json.next.safe_for_agent !== false || !json.render_plan || !json.render_plan.selected || !json.human_output_required || !json.human_output) process.exit(1);})'
PAYMENT_FALLBACK_HANDOFF_OUTPUT=$(HOME="$TMP_HOME" ITPAY_API_BASE="http://127.0.0.1:$MOCK_PORT" "$ROOT/bin/itp" buyer payment wait pi_mock_fallback --json)
printf '%s' "$PAYMENT_FALLBACK_HANDOFF_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const action=json.payment_intent && json.payment_intent.human_action; if (json.status !== "payment_handoff_required" || json.must_reply_to_human_before_next_command !== true || json.next.type !== "reply_to_human") process.exit(1); if (!action || action.kind !== "payment_qr" || action.payment_intent_id !== "pi_mock_fallback") process.exit(1); if (!json.human_output_required || !json.human_output) process.exit(1); if (!json.render_plan || json.render_plan.kind !== "payment_qr" || json.render_plan.proof_rule !== "Only payment_intent.verified proves payment.") process.exit(1); const tg=json.render_plan.platforms.telegram; if (!tg.media.length || tg.buttons.map(b=>b.intent).join(",") !== "refresh_payment_qr,check_payment_status") process.exit(1); if (!tg.openclaw_message || !tg.openclaw_message.presentation.blocks.some((b)=>b.type==="buttons")) process.exit(1); if (!json.payment_handoff.markdown.includes("mock-fallback-qr.png") || !json.payment_handoff.markdown.includes("mock-fallback-pay")) process.exit(1);})'
