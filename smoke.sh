...
printf '%s' "$AUTH_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const instruction=json.agent_instruction; const msg=instruction&&instruction.openclaw_message; const args=msg&&msg.command_args; const blocks=args&&args.presentation&&args.presentation.blocks||[]; const buttons=blocks.find((b)=>b.type==="buttons")&&blocks.find((b)=>b.type==="buttons").buttons||[]; if (json.status !== "checkout_created" || json.render_plan || json.human_output || json.human_visible_markdown || json.payment_handoff) process.exit(1); if (!json.must_send_to_human_before_next_command || !instruction || instruction.platform !== "telegram" || instruction.type !== "send_telegram_auth_handoff") process.exit(1); if (!args || args.channel !== "telegram" || args.target !== "telegram:5559456744" || !args.media || !args.message.includes("购物车和结算已创建") || !args.message.includes("首次授权/付款")) process.exit(1); if (!msg.telegram_rule.includes("openclaw message send") || !msg.telegram_rule.includes("Inline Keyboard")) process.exit(1); if (!buttons.some((button)=>button.text === "🔓 打开授权" && button.url && button.url.includes("/checkouts/chk_mock_auth"))) process.exit(1); if (!buttons.some((button)=>button.text === "✅ 我已完成，查询状态" && button.callback_data === "itp:check_checkout_status:chk_mock_auth")) process.exit(1); if (!json.after_human_response.check_status_command.includes("buyer checkout resume chk_mock_auth --host telegram --target telegram:5559456744 --json")) process.exit(1); if (JSON.stringify(json).includes("MEDIA:") || JSON.stringify(json).includes("\"label\"")) process.exit(1);})'
...
printf '%s' "$BUY_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const instruction=json.agent_instruction; const msg=instruction&&instruction.openclaw_message; const args=msg&&msg.command_args; const blocks=args&&args.presentation&&args.presentation.blocks||[]; const buttons=blocks.find((b)=>b.type==="buttons")&&blocks.find((b)=>b.type==="buttons").buttons||[]; if (json.status !== "payment_handoff_required" || json.render_plan || json.human_output || json.human_visible_markdown || json.payment_handoff || json.payment_guidance) process.exit(1); if (!json.order_summary || json.order_summary.payment_intent_id !== "pi_mock_pubg" || json.order_summary.amount_display !== "¥20.00") process.exit(1); if (!instruction || instruction.platform !== "telegram" || instruction.type !== "send_telegram_payment_handoff" || !instruction.proof_rule.includes("verified")) process.exit(1); if (!args || args.channel !== "telegram" || args.target !== "telegram:5559456744" || args.media !== "http://127.0.0.1/mock-qr.png" || !args.message.includes("购物车和结算已创建") || !args.message.includes("完成付款")) process.exit(1); if (!buttons.some((button)=>button.text === "💳 打开付款页面" && button.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (!buttons.some((button)=>button.text === "📱 手机钱包打开" && button.url === "http://127.0.0.1/mock-mobile-wallet")) process.exit(1); if (!buttons.some((button)=>button.text === "🔄 支付遇到问题 / 刷新" && button.callback_data === "itp:refresh_payment_qr:pi_mock_pubg")) process.exit(1); if (!buttons.some((button)=>button.text === "✅ 我已付款，查询状态" && button.callback_data === "itp:check_payment_status:pi_mock_pubg")) process.exit(1); if (!json.after_human_response.check_status_command.includes("buyer payment wait pi_mock_pubg --timeout 1 --host telegram --target telegram:5559456744 --json")) process.exit(1); if (JSON.stringify(json).includes("MEDIA:") || JSON.stringify(json).includes("\"label\"") || JSON.stringify(json).includes("\"action\"")) process.exit(1);})'
