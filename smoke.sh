...
printf '%s' "$AUTH_QR_OUTPUT" | node -e 'const fs=require("fs");let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const action=json.checkout && json.checkout.human_action; const selected=json.render_plan && json.render_plan.selected; if (json.status !== "checkout_created" || !action || action.kind !== "auth_qr") process.exit(1); if (!action.local_qr_path || !fs.existsSync(action.local_qr_path)) process.exit(1); if (action.local_qr_mime !== "image/png") process.exit(1); if (action.agent_display_hint.primary !== "local_qr_path") process.exit(1); if (!json.render_plan || json.render_plan.kind !== "auth_qr" || json.render_plan.proof_rule !== "This human action is not payment proof." || !selected) process.exit(1); if (!json.render_plan.platforms.telegram.buttons.length || !json.render_plan.platforms.telegram.openclaw_message) process.exit(1); if (action.qr_png_url || action.qr_image_url || action.preferred_qr_url) process.exit(1);})'
...
printf '%s' "$AUTH_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; const msg=selected && selected.openclaw_message; if (!selected || selected.platform !== "telegram" || selected.format !== "text_inline_buttons") process.exit(1); if (!selected.media.length || !selected.media[0].local_path) process.exit(1); if (!selected.links.length || !selected.links[0].url.includes("/checkouts/chk_mock_auth")) process.exit(1); if (!selected.buttons.length || !selected.buttons[0].url.includes("/checkouts/chk_mock_auth")) process.exit(1); if (!selected.text.includes("授权")) process.exit(1); if (!msg || msg.media !== selected.media[0].local_path || !msg.presentation.blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text && button.url && button.url.includes("/checkouts/chk_mock_auth")))) process.exit(1); if (JSON.stringify(msg).includes("MEDIA:") || JSON.stringify(msg).includes("\"label\"")) process.exit(1);})'
...
printf '%s' "$BUY_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; const msg=selected && selected.openclaw_message; if (!selected || selected.platform !== "telegram" || selected.format !== "photo_text_inline_buttons") process.exit(1); if (!selected.media.length || selected.media[0].fallback_url !== "http://127.0.0.1/mock-qr.png") process.exit(1); if (!selected.links.some((link)=>link.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (selected.buttons.map((button)=>button.intent).join(",") !== "refresh_payment_qr,check_payment_status") process.exit(1); if (!msg || msg.media !== "http://127.0.0.1/mock-qr.png") process.exit(1); const blocks=msg.presentation && msg.presentation.blocks || []; if (!blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text && button.url === "http://127.0.0.1/mock-pay"))) process.exit(1); if (!blocks.some((b)=>b.type==="buttons" && b.buttons.some((button)=>button.text === "我已付款，查询状态" && button.callback_data === "itp:check_payment_status:pi_mock_pubg"))) process.exit(1); if (!msg.callbacks || !msg.callbacks.check_payment_status) process.exit(1); if (JSON.stringify(msg).includes("MEDIA:") || JSON.stringify(msg).includes("\"label\"") || JSON.stringify(msg).includes("\"action\"")) process.exit(1);})'
...
printf '%s' "$BUY_UNKNOWN_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; if (!selected || selected.platform !== "current_channel_required" || !selected.telegram || !selected.telegram.openclaw_message || !selected.fallback) process.exit(1); if (selected.fallback.format !== "image_or_link_then_human_reply") process.exit(1); if (!selected.fallback.links.some((link)=>link.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (!selected.fallback.text.includes("扫码")) process.exit(1);})'
...
printf '%s' "$PAYMENT_WAIT_HANDOFF_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "payment_handoff_required" || json.next.safe_for_agent !== false || !json.render_plan || !json.render_plan.selected) process.exit(1);})'
