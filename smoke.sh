...
printf '%s' "$CONTEXT_REQUIRED" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "client_context_required" || !json.must_rerun || !json.examples.telegram.includes("--target")) process.exit(1);})'
...
printf '%s' "$AUTH_QR_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const instruction=json.agent_instruction; if (json.status !== "checkout_created" || json.render_plan || json.human_output || json.human_visible_markdown || json.payment_handoff) process.exit(1); if (!json.must_send_to_human_before_next_command || !instruction || instruction.platform !== "codex_app" || instruction.type !== "send_markdown_auth_handoff") process.exit(1); if (!instruction.markdown.includes("![ItPay auth QR](") || !instruction.markdown.includes("打开授权页面")) process.exit(1); if (!json.order_summary || json.order_summary.checkout_id !== "chk_mock_auth") process.exit(1);})'
...
printf '%s' "$BUY_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const instruction=json.agent_instruction; if (json.status !== "payment_handoff_required" || json.render_plan || json.human_output || json.human_visible_markdown || json.payment_handoff) process.exit(1); if (!json.order_summary || json.order_summary.checkout_id !== "chk_mock_pubg" || json.order_summary.payment_intent_id !== "pi_mock_pubg") process.exit(1); if (!instruction || instruction.platform !== "codex_app" || instruction.type !== "send_markdown_payment_handoff" || !instruction.markdown.includes("![ItPay payment QR](") || !instruction.markdown.includes("mock-pay")) process.exit(1); if (json.next.safe_for_agent !== false || json.next.type !== "reply_to_human") process.exit(1); if (!json.after_human_response || !json.after_human_response.check_status_command.includes("buyer payment wait pi_mock_pubg --timeout 1 --host codex --json")) process.exit(1); if (json.agent_next_actions[0] !== "send_agent_instruction_to_human") process.exit(1); if (!json.docs.some((doc)=>doc.topic==="payment-wait")) process.exit(1); if (JSON.stringify(json).includes("issue_payment_proof")) process.exit(1);})'
